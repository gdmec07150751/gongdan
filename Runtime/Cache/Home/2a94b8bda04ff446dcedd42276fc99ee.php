          ...
          <?php if($upgradeMsg[0] != null): ?><li>
           ...
           <li class="dropdown user user-menu">
              <a href="#" class="dropdown-toggle" data-toggle="dropdown">
                ...
                <span class="hidden-xs">欢迎：<?php echo ($admininfo["name"]); ?></span>
              </a>
              <ul class="dropdown-menu">
                <li class="user-footer">
                  <div class="pull-left">
                   	<a href="javascript:;" class="btn btn-default btn-flat" onclick="updatepwd(<?php echo ($admininfo["id"]); ?>)">修改密码</a>
                   	<a href="<?php echo U('Public/logout');?>" class="btn btn-default btn-flat">安全退出</a>
                  </div>
                </li>
              </ul>
            </li>
...
function updatepwd(id){
  layer.prompt({title: '修改密码'},function(val, index){
    $.ajax({
      type:'post',
      url:"<?php echo U('Public/ajaxupdatepwd');?>",
      data:{id:id,val:val},
      dataType:'json',
      success:function(rel){
        console.log(rel);
        if(rel.status==1)layer.alert(rel.msg, {icon: 1});
...
<aside class="main-sidebar" style="overflow-y:auto;">
      <section class="sidebar">
        ...
        <ul class="sidebar-menu"> 
	      <?php if(is_array($list)): foreach($list as $k=>$vo): if(!empty($vo["sub_menu"])): ?><li class="treeview">
...
<section class="content-wrapper right-side" id="riframe" style="margin:0px;padding:0px;margin-left:230px;">
    <iframe id='rightContent' name='rightContent' src="/Home/Workorder/newwork" width='100%' frameborder="0"></iframe>
</section>
...
<!-- <script src="/Public/js/IMGUP.js"></script> -->
